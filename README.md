...
via GoReleaser on every `v*` tag, for macOS and Linux on arm64 + amd64; the
Homebrew formula in [`Formula/hcb.rb`](Formula/) is regenerated by the same
release. Prebuilt tarballs are also on the
