...
The hosted OAuth bridge runs its own consent page (`GET /oauth/authorize` renders it, `POST` approves). It has to: every MCP client shares one HCB OAuth app, so HCB's own consent screen names that shared app rather than the client, and Doorkeeper skips it entirely for a user who has authorized before. The page names the exact `redirect_uri`, so anything that would make the displayed destination differ from the delivered one — a fragment, a pre-set `code`/`state`/`error`, a non-ASCII host — is rejected outright. PKCE is mandatory and no client authenticates, so the verifier is the only thing binding a minted code to its requester.
